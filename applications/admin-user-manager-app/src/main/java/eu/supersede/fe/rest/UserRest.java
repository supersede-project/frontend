...
public class UserRest
{
    ...
    public ResponseEntity<?> createUser(Authentication authentication, @RequestBody User user)
    {
        ...
        if (currentUser.getToken() != null)
        {
            try
            {
                proxy.getIFAuthenticationManager(tenant).addUser(toSecurityUser(user, tenant), user.getPassword(),
                        false);
            }
    ...
    private eu.supersede.integration.api.security.types.User toSecurityUser(User u, String tenant)
